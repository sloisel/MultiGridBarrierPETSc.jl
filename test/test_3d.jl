...
if rank == 0
    println("[DEBUG] 3D integration test starting")
    flush(stdout)
end

# Keep output tidy and aggregate at the end
ts = @testset QuietTestSet "3D integration tests" begin

for L in 1:2
    if rank == 0
        println("[DEBUG] Test L=$L: Comparing PETSc and native fem3d_petsc_solve")
        flush(stdout)
    end
...
if rank == 0
    println("[DEBUG] All 3D tests completed")
    flush(stdout)
end
...
if rank == 0
    println("Test Summary: 3D integration tests (aggregated across $(nranks) ranks)")
...
if rank == 0
    println("[DEBUG] 3D integration test file completed successfully")
    flush(stdout)
end
