...
if rank == 0
    println("[DEBUG] 1D integration test starting")
    flush(stdout)
end

# Keep output tidy and aggregate at the end
ts = @testset QuietTestSet "1D integration tests" begin

for L in 2:4
    if rank == 0
        println("[DEBUG] Test L=$L: Comparing PETSc and native fem1d_petsc_solve")
        flush(stdout)
    end

    # Solve with PETSc distributed types
    sol_petsc = MultiGridBarrierPETSc.fem1d_petsc_solve(Float64; L=L, p=1.0, verbose=false)

    # Solve with native Julia types (sequential)
    sol_native = MultiGridBarrier.fem1d_solve(Float64; L=L, p=1.0, verbose=false)
...
if rank == 0
    println("[DEBUG] All 1D tests completed")
    flush(stdout)
end
...
if rank == 0
    println("Test Summary: 1D integration tests (aggregated across $(nranks) ranks)")
...
if rank == 0
    println("[DEBUG] 1D integration test file completed successfully")
    flush(stdout)
end
